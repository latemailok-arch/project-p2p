package com.pingchat.watch.mesh
...
import com.pingchat.android.mesh.BluetoothConnectionManager
import com.pingchat.android.mesh.BluetoothConnectionManagerDelegate
...
import com.pingchat.android.util.AppConstants
...
class WearMeshService private constructor(private val context: Context) {
    ...
    var onPrivateMessage: ((com.pingchat.android.model.PingchatMessage) -> Unit)? = null
    ...
    private fun handleMessageReceived(
        message: com.pingchat.android.model.PingchatMessage
    ): Boolean = try {
        when {
            message.isPrivate -> {
     ...
     */
    fun sendFilePrivateEncrypted(recipientPeerID: String, file: com.pingchat.android.model.PingchatFilePacket) {
        serviceScope.launch {
            ...
            val transferId = com.pingchat.android.mesh.MeshPacketUtils.sha256Hex(
            ...
            while (System.currentTimeMillis() < prepDeadline) {
                when (val prep = meshCore.prepareFilePrivate(recipientPeerID, file, transferId, allowLegacyFallback = false)) {
                    is com.pingchat.android.mesh.PrivateMediaPreparation.Ready -> {
                        prep.transfer.commit()
                        return@launch
                    }
                    com.pingchat.android.mesh.PrivateMediaPreparation.AwaitingPeerState,
                    com.pingchat.android.mesh.PrivateMediaPreparation.NeedsHandshake -> {
                        if (prep == com.pingchat.android.mesh.PrivateMediaPreparation.NeedsHandshake) {
                            try { initiateNoiseHandshake(recipientPeerID) } catch (_: Exception) { }
                        }
                        delay(500)
                    }
    ...
    private fun prefs() = context.getSharedPreferences("pingchat_watch_prefs", Context.MODE_PRIVATE)
