...
class BluetoothConnectionManager(
    ...
    private fun isGattServerEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { com.pingchat.android.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true })
    }

    private fun isGattClientEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { com.pingchat.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true })
    }
    ...
    init {
        ...
        try {
            val dbg = com.pingchat.android.ui.debug.DebugSettingsManager.getInstance()
     ...
     */
    fun sendPacketToPeer(peerID: String, packet: PingchatPacket): Boolean {
        if (!isActive || !isBleTransportEnabled()) return false
    ...
    fun sendPacketToLink(deviceAddress: String, linkID: String, packet: PingchatPacket): Boolean {
        if (!isActive || !isBleTransportEnabled()) return false
