...
/**
 * Shared transport send wrapper that applies pingchat packet fragmentation and
...
class FragmentingPacketSender(
    ...
    private fun packetsForTransport(routed: RoutedPacket): List<PingchatPacket>? {
        routed.preparedPackets?.let { prepared ->
            if (prepared.isEmpty() ||
                prepared.size > com.pingchat.android.util.AppConstants.Fragmentation.MAX_FRAGMENTS_PER_ID) {
