...
import com.pingchat.android.model.FragmentPayload
...
class FragmentManager {
    ...
    companion object {
        ...
        private const val FRAGMENT_SIZE_THRESHOLD = com.pingchat.android.util.AppConstants.Fragmentation.FRAGMENT_SIZE_THRESHOLD // Matches iOS: if data.count > 512
        private const val MAX_FRAGMENT_SIZE = com.pingchat.android.util.AppConstants.Fragmentation.MAX_FRAGMENT_SIZE        // Matches iOS: maxFragmentSize = 469 
        private const val FRAGMENT_TIMEOUT = com.pingchat.android.util.AppConstants.Fragmentation.FRAGMENT_TIMEOUT_MS     // Matches iOS: 30 seconds cleanup
        private const val CLEANUP_INTERVAL = com.pingchat.android.util.AppConstants.Fragmentation.CLEANUP_INTERVAL_MS     // 10 seconds cleanup check
    }
    ...
    fun createFragments(packet: PingchatPacket): List<PingchatPacket> =
     ...
     * UInt16 wire limit.
     */
    fun createFragments(packet: PingchatPacket, maxFragments: Int): List<PingchatPacket> {
        try {
            ...
            val fragments = mutableListOf<PingchatPacket>()
            ...
            for (index in fragmentChunks.indices) {
                ...
                val fragmentPacket = PingchatPacket(
     ...
     */
    fun handleFragment(packet: PingchatPacket): PingchatPacket? {
        ...
        try {
            ...
            val maxFragments = com.pingchat.android.util.AppConstants.Fragmentation.MAX_FRAGMENTS_PER_ID
            ...
            synchronized(fragmentStateLock) {
                ...
                if (isNewSet) {
                    val maxActive = com.pingchat.android.util.AppConstants.Fragmentation.MAX_ACTIVE_FRAGMENT_SETS
                ...
                val maxTotalBytes = com.pingchat.android.util.AppConstants.Fragmentation.MAX_FRAGMENT_TOTAL_BYTES
                ...
                val maxGlobalBytes = com.pingchat.android.util.AppConstants.Fragmentation.MAX_GLOBAL_FRAGMENT_TOTAL_BYTES
                ...
                if (fragmentMap.size == expectedTotal) {
                    ...
                    val originalPacket = PingchatPacket.fromBinaryData(reassembledData.toByteArray())
...
interface FragmentManagerDelegate {
    fun onPacketReassembled(packet: PingchatPacket)
}
