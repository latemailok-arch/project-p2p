...
class MeshCore(
    ...
    data class Hooks(
         ...
         */
        val onMessageReceived: ((PingchatMessage) -> Boolean)? = null,
    ...
    init {
        ...
        if (sharedGossipManager == null) {
            gossipSyncManager.delegate = object : GossipSyncManager.Delegate {
                ...
                override fun sendPacketToPeer(peerID: String, packet: PingchatPacket) {
                    transport.sendPacketToPeer(peerID, packet)
                    TransportBridgeService.sendToPeer(transport.id, peerID, packet)
                }

                override fun signPacketForBroadcast(packet: PingchatPacket): PingchatPacket {
    ...
    private fun setupDelegates() {
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                try { com.pingchat.android.services.AppStateStore.setTransportPeers(transport.id, peerIDs) } catch (_: Exception) { }
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerInfoFromVerifiedAnnouncement(
                ...
                isVerified: Boolean,
                capabilities: com.pingchat.android.model.PeerCapabilities?
            ): Boolean {
            ...
            override fun onDeliveryAckReceived(messageID: String, peerID: String) {
                try {
                    com.pingchat.android.services.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        com.pingchat.android.model.DeliveryStatus.Delivered(peerID, java.util.Date())
                    )
                } catch (_: Exception) { }
            ...
            override fun onReadReceiptReceived(messageID: String, peerID: String) {
                try {
                    com.pingchat.android.services.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        com.pingchat.android.model.DeliveryStatus.Read(peerID, java.util.Date())
                    )
                } catch (_: Exception) { }
    ...
    fun sendFileBroadcast(file: PingchatFilePacket) {
        try {
    ...
    fun sendFilePrivate(recipientPeerID: String, file: PingchatFilePacket) {
        val payload = file.encode() ?: return
    ...
    fun sendVerifyChallenge(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val payload = NoisePayload(
            ...
            data = com.pingchat.android.services.VerificationService.buildVerifyChallenge(noiseKeyHex, nonceA)
        )
        sendNoisePayloadToPeer(payload, peerID)
    }

    fun sendVerifyResponse(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val tlv = com.pingchat.android.services.VerificationService.buildVerifyResponse(noiseKeyHex, nonceA) ?: return
        val payload = NoisePayload(
    ...
    private fun buildAnnouncementPayload(announcement: IdentityAnnouncement, nickname: String): ByteArray? {
        ...
        try {
            if (directPeersForGossip.isNotEmpty()) {
                tlvPayload += com.pingchat.android.services.meshgraph.GossipTLV.encodeNeighbors(directPeersForGossip)
            }
    ...
    private fun getDirectPeerIDsForGossip(): List<String> {
        return try {
            ...
            try { com.pingchat.android.services.AppStateStore.setTransportDirectPeers(transport.id, localDirect) } catch (_: Exception) { }
            val union = try {
                com.pingchat.android.services.AppStateStore.getDirectPeers().ifEmpty { localDirect }
