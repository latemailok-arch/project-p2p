package com.pingchat.android.protocol
...
data class PingchatPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = PingchatPacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            route = route,
            ttl = com.pingchat.android.util.AppConstants.SYNC_TTL_HOPS, // Use fixed TTL=0 for signing to ensure relay compatibility
    ...
    companion object {
        fun fromBinaryData(data: ByteArray): PingchatPacket? {
...
object BinaryProtocol {
    ...
    fun encode(packet: PingchatPacket, padding: Boolean = true): ByteArray? {
        try {
            val maxPayloadLength = com.pingchat.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH
    ...
    fun decode(data: ByteArray): PingchatPacket? =
    ...
    ): PingchatPacket? = decode(data, decompress)
    ...
    private fun decodeCore(
        ...
        try {
            ...
            if (payloadLength > com.pingchat.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length ${payloadLength} exceeds maximum allowed (${com.pingchat.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
            ...
            val payload = if (isCompressed) {
                ...
                val maxExpandedSize = com.pingchat.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH
