...
import com.pingchat.android.BuildConfig
import com.pingchat.android.net.ArtiTorManager
import com.pingchat.android.net.OkHttpProvider
...
class UniversalApkManager(
    ...
    companion object {
        ...
        private const val APK_FILE_PREFIX = "pingchat-universal-"
    ...
    private suspend fun executeDownloadRequest(
        ...
        val request = Request.Builder()
            ...
            .url(endpointUrl)
            .addHeader("User-Agent", "PingChat-Android")
            .apply {
                if (existingBytes > 0L) {
    ...
    private fun validateDownloadedApk(tempFile: File, source: ApkDownloadSource) {
        if (!verifyApkSignature(tempFile)) {
            clearPartialDownload()
            throw ApkDownloadException(
                message = "APK from ${source.id} is not signed by a trusted PingChat release key.",
    ...
    private fun downloadedVersionName(apkFile: File): String {
        ...
        if (packageInfo.packageName != context.packageName) {
            invalidDownloadedApk(ApkDownloadFailureReason.NotPingchat, "wrong package")
        }
