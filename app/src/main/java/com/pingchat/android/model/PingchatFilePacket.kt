package com.pingchat.android.model

import java.nio.ByteBuffer
import java.nio.ByteOrder

/**
 * PingchatFilePacket: TLV-encoded file transfer payload for BLE mesh.
 * TLVs:
 *  - 0x01: filename (UTF-8)
 ...
 *
 * Note: The outer PingchatPacket uses version 2 (4-byte payload length), so this
...
data class PingchatFilePacket(
    ...
    fun encode(): ByteArray? {
        try {
            android.util.Log.d("PingchatFilePacket", "🔄 Encoding: name=$fileName, size=$fileSize, mime=$mimeType")
        ...
        if (nameBytes.size > 0xFFFF || mimeBytes.size > 0xFFFF) {
                android.util.Log.e("PingchatFilePacket", "❌ TLV field too large: name=${nameBytes.size}, mime=${mimeBytes.size} (max: 65535)")
                return null
            }
            if (content.size > 0xFFFF) {
                android.util.Log.d("PingchatFilePacket", "📦 Content exceeds 65535 bytes (${content.size}); will be split into multiple CONTENT TLVs")
            } else {
                android.util.Log.d("PingchatFilePacket", "📏 TLV sizes OK: name=${nameBytes.size}, mime=${mimeBytes.size}, content=${content.size}")
            }
        ...
        val result = buf.array()
            android.util.Log.d("PingchatFilePacket", "✅ Encoded successfully: ${result.size} bytes total")
            return result
        } catch (e: Exception) {
            android.util.Log.e("PingchatFilePacket", "❌ Encoding failed: ${e.message}", e)
            return null
        }
    }

    companion object {
        fun decode(data: ByteArray): PingchatFilePacket? {
            android.util.Log.d("PingchatFilePacket", "🔄 Decoding ${data.size} bytes")
            try {
                var off = 0
                ...
                if (skippedUnknownTLVs > 0) {
                    android.util.Log.d("PingchatFilePacket", "⏭️ Skipped $skippedUnknownTLVs unknown TLV(s)")
                }
                val n = name ?: return null
                val c = contentBytes ?: return null
                val s = size ?: c.size.toLong()
                val m = mime ?: "application/octet-stream"
                val result = PingchatFilePacket(n, s, m, c)
                android.util.Log.d("PingchatFilePacket", "✅ Decoded: name=$n, size=$s, mime=$m, content=${c.size} bytes")
                return result
            } catch (e: Exception) {
                android.util.Log.e("PingchatFilePacket", "❌ Decoding failed: ${e.message}", e)
                return null
            }
        }
    }
