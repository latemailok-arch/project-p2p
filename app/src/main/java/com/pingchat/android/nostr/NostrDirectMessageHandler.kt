...
import com.pingchat.android.services.SeenMessageStore
import com.pingchat.android.ui.ChatState
import com.pingchat.android.ui.PrivateChatManager
import com.pingchat.android.ui.PrivateMessageOrigin
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.pingchat.android.ui.DataManager,
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch {
            try {
                ...
                if (!content.startsWith("pingchat1:")) return@launch

                val base64Content = content.removePrefix("pingchat1:")
                val packetData = base64URLDecode(base64Content) ?: return@launch
                val packet = PingchatPacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.pingchat.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.pingchat.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = PingchatFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.pingchat.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = PingchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
                    ...
                    if (!admitted) {
                        com.pingchat.android.features.file.FileUtils.deleteStoredMediaPaths(
                            application,
                            listOf(savedPath)
                        )
                    }
    ...
    private suspend fun handleFavoriteControl(
        ...
        return try {
            ...
            val systemMessage = PingchatMessage(
                sender = "system",
