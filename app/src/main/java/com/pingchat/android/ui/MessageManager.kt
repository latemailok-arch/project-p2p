...
class MessageManager(private val state: ChatState) {
    ...
    private val MESSAGE_DEDUP_TIMEOUT = com.pingchat.android.util.AppConstants.UI.MESSAGE_DEDUP_TIMEOUT_MS // 30 seconds
    private val SYSTEM_EVENT_DEDUP_TIMEOUT = com.pingchat.android.util.AppConstants.UI.SYSTEM_EVENT_DEDUP_TIMEOUT_MS // 5 seconds
    
    // MARK: - Public Message Management
    
    fun addMessage(message: PingchatMessage) {
        ...
        try { com.pingchat.android.services.AppStateStore.addPublicMessage(message) } catch (_: Exception) { }
    }
    ...
    fun addChannelMessage(channel: String, message: PingchatMessage) {
        ...
        try { com.pingchat.android.services.AppStateStore.addChannelMessage(channel, message) } catch (_: Exception) { }
        ...
        val viewingGeohashChannel = try {
            if (channel.startsWith("geo:")) {
                ...
                selected is com.pingchat.android.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
            } else false
    ...
    fun addPrivateMessage(peerID: String, message: PingchatMessage) {
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        val accepted = try {
            com.pingchat.android.services.AppStateStore.addPrivateMessage(
                conversationID,
                message
            )
        } catch (_: Exception) {
    ...
    suspend fun addPrivateMessageDurably(
        ...
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        val accepted = try {
            com.pingchat.android.services.AppStateStore.addPrivateMessageDurably(
    ...
    fun addPrivateMessageNoUnread(peerID: String, message: PingchatMessage) {
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        val accepted = try {
            com.pingchat.android.services.AppStateStore.addPrivateMessage(
                peerID = conversationID,
    ...
    fun clearPrivateMessages(peerID: String) {
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        com.pingchat.android.services.AppStateStore.deletePrivateConversation(conversationID)
     ...
     */
    fun generateMessageKey(message: PingchatMessage): String {
    ...
    fun updateMessageDeliveryStatus(messageID: String, status: DeliveryStatus) {
        ...
        if (updated) {
            ...
            try { com.pingchat.android.services.AppStateStore.updatePrivateMessageStatus(messageID, status) } catch (_: Exception) { }
        }
    ...
    fun removeMessageById(messageID: String) {
        ...
        run {
            ...
            if (changed) {
                ...
                com.pingchat.android.services.AppStateStore.removePrivateMessage(messageID)
            }
        }
        // Channels
        run {
