...
import com.pingchat.android.services.PrivateMessageArrivalOrder
...
internal data class ConversationSummary(
    ...
    val latestMessageType: PingchatMessageType,
...
internal fun buildConversationSummaries(
    ...
    privateChats: Map<String, List<PingchatMessage>>,
    currentUserIdentifiers: Set<String>,
    canonicalize: (String) -> String,
    isMessageRead: (PingchatMessage) -> Boolean,
    ...
    val messagesByCanonicalID = linkedMapOf<String, MutableList<PingchatMessage>>()
    ...
    return messagesByCanonicalID.mapNotNull { (normalizedConversationID, sourceMessages) ->
        ...
        fun activityOrder(message: PingchatMessage): Long =
            PrivateMessageArrivalOrder.sequenceOf(message.id) ?: message.timestamp.time

        val latest = messages.maxWithOrNull(
            compareBy<PingchatMessage>(::activityOrder).thenBy { it.id }
        ) ?: return@mapNotNull null
        fun isOutgoing(message: PingchatMessage): Boolean =
...
private fun PingchatMessage.conversationPreview(): String {
    val preview = when (type) {
        PingchatMessageType.File -> content
            .substringAfterLast('/')
            .substringAfterLast('\\')
        else -> content
    }
