...
class FragmentManagerTest {
    ...
    fun `test fragmentation size difference with and without route`() {
        ...
        // 1. Without route
        val packetNoRoute = PingchatPacket(
            version = 1u,
            type = MessageType.MESSAGE.value,
            senderID = hexStringToByteArray(senderID),
            recipientID = hexStringToByteArray(recipientID),
        ...
        val packetWithRoute = PingchatPacket(
            version = 2u,
            type = MessageType.MESSAGE.value,
            senderID = hexStringToByteArray(senderID),
            recipientID = hexStringToByteArray(recipientID),
    ...
    fun `test reassembly`() {
        ...
        val originalPacket = PingchatPacket(
            version = 1u,
        ...
        var reassembledPacket: PingchatPacket? = null
    ...
    fun `generic public packet retains a 257 fragment outbound plan`() {
        ...
        fun plan(contentSize: Int): List<PingchatPacket> = fragmentManager.createFragments(
            PingchatPacket(
                version = 2u,
                type = MessageType.FILE_TRANSFER.value,
                senderID = hexStringToByteArray(senderID),
                recipientID = com.pingchat.android.protocol.SpecialRecipients.BROADCAST,
                timestamp = 1u,
