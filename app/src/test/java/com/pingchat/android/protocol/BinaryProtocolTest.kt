...
class BinaryProtocolTest {
    ...
    @Test
    fun `PingchatPacket toBinaryData propagates padding flag`() {
     ...
     * anything it can't parse as a valid PingchatPacket.
     */
    @Test
    ...
    fun `sender ID is padded or truncated to exactly 8 bytes`() {
        ...
        val shortPacket = PingchatPacket(
            version = 1u,
    ...
    fun `v2 expanded payload at exact maximum passes bound without allocating output`() {
        val max = com.pingchat.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH
    ...
    fun `new sender refuses legacy 11 MiB public file transfer before transmission`() {
        ...
        val filePayload = PingchatFilePacket(
    ...
    private fun roundTrip(packet: PingchatPacket): PingchatPacket {
    ...
    private fun assertPacketEquals(expected: PingchatPacket, actual: PingchatPacket) {
