...
class FileTransferTest {
    ...
    fun `encode and decode file packet with all fields should preserve data`() {
        ...
        val originalPacket = PingchatFilePacket(
            fileName = "test.png",
            mimeType = "image/png",
            fileSize = 1024000,
        ...
        val decoded = PingchatFilePacket.decode(encoded!!)
    ...
    fun `decode should skip unknown TLV types instead of dropping the whole file`() {
        ...
        // When: Decoding
        val decoded = PingchatFilePacket.decode(buf.array())
    ...
    fun `decode should reject an incomplete TLV header after an unknown extension`() {
        ...
        assertNull(PingchatFilePacket.decode(packetWithUnknownExtension + byteArrayOf(0x06)))
        assertNull(PingchatFilePacket.decode(packetWithUnknownExtension + byteArrayOf(0x06, 0x00)))
    }
    ...
    fun `replaceFilePathInContent should correctly format content markers for different file types`() {
        ...
        val imageMessage = PingchatMessage(
            id = "test1",
            sender = "alice",
            senderPeerID = "12345678",
            content = "/data/user/0/com.pingchat.android/files/images/photo.jpg",
            type = PingchatMessageType.Image,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val audioMessage = PingchatMessage(
            id = "test2",
            sender = "bob",
            senderPeerID = "87654321",
            content = "/data/user/0/com.pingchat.android/files/audio/voice.amr",
            type = PingchatMessageType.Audio,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val fileMessage = PingchatMessage(
            id = "test3",
            sender = "charlie",
            senderPeerID = "11223344",
            content = "/data/user/0/com.pingchat.android/files/documents/document.pdf",
            type = PingchatMessageType.File,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )
        ...
        result = result.replace(
            "/data/user/0/com.pingchat.android/files/images/photo.jpg",
            "[image] photo.jpg"
        )
    ...
    fun `media picker should handle file size limits correctly`() {
        ...
        val maxAllowedSize = com.pingchat.android.util.AppConstants.Media.MAX_FILE_SIZE_BYTES
